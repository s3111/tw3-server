...
class AccController{
    async create(req,res){
        ...
        let {server,login,pass,} = req.body
        const myId = 4
            ...
            if (token) {
                ...
                if (decoded.email) {
                    ...
                    if(user.isActivated) {
                        ...
                        await Bot.create({ server, login, pass, userId: myId, created_by: user.id, pool, proxy: JSON.stringify([])})
                        console.log('Acc created:',server,login,pass, user.id)
        ...
        return res.json({balance,isError,errorMsg,accCreated})
    }
            ...
            if (token) {
                ...
                if (decoded.email) {
                    ...
                    bots = await Bot.findAll({
                        ...
                        attributes: ['id','server','login','ban','created_at','deleted_at' ],
                        raw:true,
                        order: [['id','DESC']]
                    })
                    bots = bots.map(item => {
